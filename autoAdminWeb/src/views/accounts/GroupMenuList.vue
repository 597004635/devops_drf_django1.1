<template>
    <div class="user-list-container">
      <div style="margin-bottom:20px;font-size:14px;">{{ '修改角色的前端menu：' + name }}</div>
        <el-tree :data="data" show-checkbox default-expand-all node-key="id" ref="tree" highlight-current :render-content="renderContent" />
    </div>
</template>

<script>
import { getGroupMenuList, deleteGroupMenu, updateGroupMenu } from '@/api/users'
export default {
  data() {
    return {
      data: []
