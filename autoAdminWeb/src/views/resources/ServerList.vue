<template>
  <div>
    <el-form ref="searchForm" :model="searchForm" :inline="true">
      <el-form-item>
        <el-select v-model="searchForm.idc" style="width: 150px">
          <el-option v-for="(item, index) in searchIdcsList" :key="index" :label="item.idc_name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item>
        <el-select v-model="searchForm.cabinet" style="width: 150px">
          <el-option v-for="(item, index) in searchCabinetList" :key="index" :label="item.name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item>
        <el-select v-model="searchForm.service_id" style="width: 150px">
          <el-option v-for="(item, index) in searchServiceIdList" :key="index" :label="item.service_name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item>
        <el-select v-model="searchForm.server_purpose" style="width: 150px">
          <el-option v-for="(item, index) in searchServerPurposeList" :key="index" :label="item.idc_name" :value="item.id" />
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <el-table v-loading="loading" :data="serverList" class="table" element-loading-text="拼命加载中" border>
      ...
      <el-table-column prop="idc.name" label="机房" align="center" />
      <el-table-column prop="service.name" label="业务线" align="center" />
      <el-table-column prop="server_purpose.name" label="产品线" align="center" />
      <el-table-column prop="hostname" label="主机名" align="center" />
      <el-table-column prop="manage_ip" label="管理IP" align="center" />
      <el-table-column prop="device_type.name" label="机型类型" align="center" />
      <el-table-column prop="status" label="状态" width="60" align="center" />
      <el-table-column prop="last_check" label="LAST CHECK" width="155" align="center" />
    ...
    <!-- 修改服务器开始 -->
    <el-dialog :visible.sync="changeServerVisible" title="修改服务器">
      <el-form ref="changeServerForm" :model="changeServerForm" :rules="changeServerFormRules" label-width="70px">
        ...
        <el-form-item label="机房" prop="idc">
          <el-select v-model="changeServerForm.idc" @change="changeIdc">
            <el-option v-for="(item, index) in idcsList" :key="index" :label="item.idc_name" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="机柜" prop="cabinet">
          <el-select v-model="changeServerForm.cabinet">
            <el-option v-for="(item, index) in cabinetList" :key="index" :label="item.name" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="业务线" prop="service">
          <el-select v-model="changeServerForm.service" @change="changeService">
            <el-option v-for="(item, index) in serviceIdList" :key="index" :label="item.service_name" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="产品线" prop="server_purpose">
          <el-select v-model="changeServerForm.server_purpose">
            <el-option v-for="(item, index) in serverPurposeList" :key="index" :label="item.service_name" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="备注">
...
<script>
import { getServerList, deleteServer, getIdcsList, getCabinetList, getProductLevel, updateServer, getNetworkDeviceList, getIpList } from '@/api/resources'
...
export default {
  ...
  computed: {
    searchIdcsList() {
      return [{ id: '', idc_name: '所在机房' }].concat(this.idcsList)
    },
    searchServiceIdList() {
